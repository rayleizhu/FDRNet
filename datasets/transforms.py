...
class Denormalize(object):
    ...
    denormalize a image tensor(float) for visualization
    """
    def __init__(self, mean=[0.5, 0.5, 0.5], std=[0.5, 0.5, 0.5]):
        # self.mean = torch.Tensor(mean).view(3, 1, 1)
        # self.std = torch.Tensor(std).view(3, 1, 1)
        inv_std = [1/x for x in std ]
        inv_mean = [-m*s for (m, s) in zip(mean, inv_std)]
        self.denorm = transforms.Normalize(mean=inv_mean, std=inv_std)

    def __call__(self, x):
        # x : Float tensor image of size (C, H, W) or (B, C, H, W) to be normalized.
